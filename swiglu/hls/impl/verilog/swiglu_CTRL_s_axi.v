...
localparam
    ...
    ADDR_W_DOWN2_DATA_0         = 7'h34,
    ADDR_W_DOWN2_DATA_1         = 7'h38,
    ADDR_W_DOWN2_CTRL           = 7'h3c,
    ADDR_X_BATCH_DATA_0         = 7'h40,
    ADDR_X_BATCH_DATA_1         = 7'h44,
    ADDR_X_BATCH_CTRL           = 7'h48,
    ADDR_OUT_BATCH_DATA_0       = 7'h4c,
    ADDR_OUT_BATCH_DATA_1       = 7'h50,
    ADDR_OUT_BATCH_CTRL         = 7'h54,
    ADDR_DOWN_QUANT_MODE_DATA_0 = 7'h58,
    ADDR_DOWN_QUANT_MODE_CTRL   = 7'h5c,
    ADDR_X_SCALE_DATA_0         = 7'h60,
    ADDR_X_SCALE_CTRL           = 7'h64,
    WRIDLE                      = 2'd0,
    WRDATA                      = 2'd1,
    WRRESP                      = 2'd2,
    WRRESET                     = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_W_DOWN2_DATA_0: begin
                    rdata <= int_W_down2[31:0];
                end
                ADDR_W_DOWN2_DATA_1: begin
                    rdata <= int_W_down2[63:32];
                end
                ...
                ADDR_OUT_BATCH_DATA_1: begin
                    rdata <= int_out_batch[63:32];
                end
                ADDR_DOWN_QUANT_MODE_DATA_0: begin
                    rdata <= int_down_quant_mode[31:0];
                end
                ADDR_X_SCALE_DATA_0: begin
                    rdata <= int_x_scale[31:0];
                end
            endcase
        end
    end
...
assign W                 = int_W;
assign V                 = int_V;
assign W_down            = int_W_down;
assign W_down2           = int_W_down2;
assign x_batch           = int_x_batch;
assign out_batch         = int_out_batch;
assign down_quant_mode   = int_down_quant_mode;
assign x_scale           = int_x_scale;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_W_DOWN2_DATA_0)
            int_W_down2[31:0] <= (WDATA[31:0] & wmask) | (int_W_down2[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_W_DOWN2_DATA_1)
            int_W_down2[63:32] <= (WDATA[31:0] & wmask) | (int_W_down2[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_out_batch[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUT_BATCH_DATA_0)
            int_out_batch[31:0] <= (WDATA[31:0] & wmask) | (int_out_batch[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_out_batch[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUT_BATCH_DATA_1)
            int_out_batch[63:32] <= (WDATA[31:0] & wmask) | (int_out_batch[63:32] & ~wmask);
    end
end

// int_down_quant_mode[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_down_quant_mode[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_DOWN_QUANT_MODE_DATA_0)
            int_down_quant_mode[31:0] <= (WDATA[31:0] & wmask) | (int_down_quant_mode[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_X_SCALE_DATA_0)
            int_x_scale[31:0] <= (WDATA[31:0] & wmask) | (int_x_scale[31:0] & ~wmask);
    end
end
