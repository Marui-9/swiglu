...
#pragma HLS ARRAY_PARTITION variable=x_local_1 dim=2 complete
    ...
    COMPUTE_X1: for (int row = 0; row < FFN_DIM; row += 4) {
        load_row_wv_urm(W_wide, row,     nib_r0, d0, dmin0, sc60, mn60);
        load_row_wv_urm(W_wide, row + 1, nib_r1, d1, dmin1, sc61, mn61);
        load_row_wv_urm(W_wide, row + 2, nib_r2, d2, dmin2, sc62, mn62);
        load_row_wv_urm(W_wide, row + 3, nib_r3, d3, dmin3, sc63, mn63);
...
#pragma HLS ARRAY_PARTITION variable=x_local_2 dim=2 complete
    ...
    COMPUTE_X2: for (int row = 0; row < FFN_DIM; row += 4) {
        load_row_wv_urm(V_wide, row,     nib_r0, d0, dmin0, sc60, mn60);
        load_row_wv_urm(V_wide, row + 1, nib_r1, d1, dmin1, sc61, mn61);
        load_row_wv_urm(V_wide, row + 2, nib_r2, d2, dmin2, sc62, mn62);
        load_row_wv_urm(V_wide, row + 3, nib_r3, d3, dmin3, sc63, mn63);
