...
\begin{IEEEkeywords}
FPGA, Edge AI, LFM2.5, Hardware Acceleration
\end{IEEEkeywords}
...
The results indicate that offloading the SwiGLU feed-forward block to FPGA is beneficial because it targets a compute-dominant kernel, much as prior transformer accelerators have focused on FFN/MLP and attention substructures as the most profitable hardware targets %\cite{FTRANS} 
\cite{lu2020hw-mha}. In the present case, the FPGA+CPU configuration improves performance-per-watt most clearly at lower thread counts (T1/T2), indicating that the accelerator is effective when host-side contention is limited and the cost of orchestration can be amortized. By contrast, at T3/T4 the CPU Only path remains faster, which suggests that the incremental benefit of SwiGLU acceleration is offset by transfer overhead, synchronization, and the inability of the surrounding execution pipeline to supply data to the programmable logic efficiently enough. Results reported using larger FPGA platforms, greater memory bandwidth and fabric capacity help kernel-level gains and end-to-end speedup \cite{FTRANS}.

A further practical observation is that the design operates close to the resource limit of the KV260 device: most usable LUT/logic budget is already occupied, and attempts to increase parallelism primarily intensify routing pressure and timing-closure difficulty rather than yielding proportional throughput gains. Consequently, the present results are better interpreted as characterizing the trade-off space of edge heterogeneous inference rather than as an isolated acceleration result. Similar to prior FPGA transformer studies, hardware efficiency is not determined only by the mapped operator, but also by whether the platform can sustain the required on-chip reuse and off-chip bandwidth without excessive control overhead \cite{FTRANS}. These observations suggest that for edge transformer inference, compute offload of a dominant block such as SwiGLU is valuable, but its system-level benefit depends on co-optimizing weight movement, memory layout, and access orchestration; otherwise, improvements in compute energy per token do not fully translate into proportional end-to-end throughput gains.
