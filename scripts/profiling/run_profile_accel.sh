#!/bin/bash
# run_profile_accel.sh — FPGA+CPU benchmark with power, RAM, and throughput
# Usage: sudo ./scripts/run_profile_accel.sh <threads> [repeat]
# Run from ~/llama.cpp/
...
echo "=== FPGA+CPU | T=$THREADS | R=$REPEAT ===" | tee "$LOG"
echo "" >> "$LOG"
...
# ─── run benchmark ──────────────────────────────────────────────────
cd "$LLAMA_DIR"
echo "[$(date +%T)] Running llama-bench (FPGA+CPU)..." | tee -a "$LOG"
sudo env LLAMA_SWIHW=1 SWIGLU_DEBUG=1 ./build/bin/llama-bench \
...
ELAPSED_VALS=$(grep -oP 'elapsed=\K[0-9]+' "$LOG" | awk '$1>0' || true)
if [ -n "$ELAPSED_VALS" ]; then
  COUNT=$(echo "$ELAPSED_VALS" | wc -l)
  SUM=$(echo "$ELAPSED_VALS" | paste -sd+ | bc)
  AVG=$((SUM / COUNT))
  MIN=$(echo "$ELAPSED_VALS" | sort -n | head -1)
  MAX=$(echo "$ELAPSED_VALS" | sort -n | tail -1)
  echo "FPGA per-call decode (ms): avg=$AVG  min=$MIN  max=$MAX  samples=$COUNT" | tee -a "$LOG"
fi
...
if [ -n "$MEM_MIN" ] && [ "$MEM_BASELINE" -gt 0 ]; then
  ...
  echo "Peak memory used: ${USED_MB} MB (system MemAvailable delta, includes udmabuf)" | tee -a "$LOG"
fi
