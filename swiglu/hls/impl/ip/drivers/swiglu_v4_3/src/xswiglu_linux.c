...
typedef struct {
    ...
    XSwiglu_uio_map maps[ MAX_UIO_MAPS ];
} XSwiglu_uio_info;

/***************** Variable Definitions **************************************/
static XSwiglu_uio_info uio_info;
...
static int uio_info_read_name(XSwiglu_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSwiglu_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSwiglu_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSwiglu_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSwiglu_Initialize(XSwiglu *InstancePtr, const char* InstanceName) {
	XSwiglu_uio_info *InfoPtr = &uio_info;
