...
This work presents a systems-driven approach to enabling conversational audio inference on resource-constrained edge devices. This is achieved through a custom FPGA accelerator for the LFM2.5-1.2B-Thinking model by Liquid AI designed to offload the heaviest part of inference to reduce latency and improve performance per watt. The proposed implementation enables a comprehensive evaluation of the interaction between computation and data movement in transformer feed-forward layers under edge constraints. Results show a significant improvement in performance per watt while maintaining compatibility with existing inference frameworks. In single-threaded execution the FPGA+CPU path reached performance-per-watt gains of +57\% (prefill) and +44\% (decode) over the CPU baseline.
...
automatic speech recognition, language modelling, and text-to-speech
...
Standard speech processing follows a modular pipeline in which an automatic
speech recognition (ASR) module converts audio to text, a natural language
understanding component extracts intent, a language model generates a textual
response, and a text-to-speech (TTS) engine synthesises the output waveform.
This decomposition enables each stage to be optimised independently for
...
received, reducing the time to first token compared to batch-based processing.
The aggregate parameter count across three or four separate models can reach
...
LFM2-Audio~\cite{amini2025lfm2technicalreport} exemplifies this trend: a 1.5\,B
...
simplifying the deployment footprint.
Profiling several conversational models in the GGUF format reveals substantial
...
(1.18\,B), positioning it closest to the resource envelope of embedded
...
detection can run continuously on microcontrollers drawing under 1\,W, while more
...
and word-based recognition across microcontroller-class devices (Cortex-M4/M7
at 64--480\,MHz, with hundreds of kilobytes of SRAM) identify specific architectures
...
Despite this progress, existing work has largely addressed individual pipeline
...
ASR front-end, and the TTS output stage must all complete within the latency and
...
The profiling covers parameter count, unified memory (UMA) footprint, memory
bandwidth requirements, and estimated compute throughput, the same metrics
...
lightest deployment envelope (mean 13.41\,GB/s bandwidth, 134\,GFLOPS compute, 826\,MiB UMA)
and that parameter count alone is an insufficient predictor of edge feasibility.
...
Recent work on the AMD-Xilinx Kria KV260 demonstrates the feasibility of
...
which together account for the majority of inference latency and bandwidth
consumption in transformer workloads.
...
Two GEMV operations compute $X_1 = x \cdot W$ and $X_2 = x \cdot V$ in parallel
...
words per row, $\times$\,4 rows = 320 total), eliminating three burst-setup overheads per iteration compared
to the previous four-call design.
Nibble extraction uses compile-time \texttt{.range()} indexing on wide 128-bit
BRAM tiles, eliminating the runtime multiplexer trees that dominated LUT
consumption in the earlier K=2 design.
...
The fused gate operation runs entirely on-chip from the X1\_cache and X2\_cache
...
The gated activation is projected back to model dimension via
...
Unlike the merged single burst of Stages~2a/2b, the output path issues four
sequential per-row AXI bursts of 320 words each; these loads account for 60\%
of the per-iteration budget (1,908 of 3,165 cycles), making the stage load-bound.
Output nibbles are stored in 32-bit tiles (4 groups $\times$ 4 rows) rather than
128-bit tiles, avoiding routing congestion from wide-BRAM fanout to 32 blocks
at 87\% LUT occupancy.
...
% Stage 2
\node[stageLow, below=7mm of x1buf] (x1) {\textbf{Stage 2a}\\$X_1=xW_{gate}$\\1.54 Million cycles / 6.14\,ms};
\node[stageLow, below=7mm of x2buf] (x2) {\textbf{Stage 2b}\\$X_2=xW_{up}$\\1.54 Million cycles / 6.14\,ms};
...
\hline
\multicolumn{7}{\textit{KV260 (4$\times$ Cortex-A53, CPU Only: Q4\_K\_M \textbar{} FPGA+CPU: Q4\_K, 250\,MHz FPGA fabric, power from onboard telemetry\textsuperscript{a})}} \\
\hline
...
\hline
T1 & CPU Only & 0.83 & 0.67\textsuperscript{c} & 0.62 & 1.339 & 1.081 \\
T2 & CPU Only & 1.61 & 1.28\textsuperscript{c} & 0.80 & 2.013 & 1.600 \\
\hline
\end{tabular}

\par\vspace{4pt}
{\footnotesize
\textsuperscript{a}xmutil platformstats, 1\,Hz sampling concurrent with benchmark.
...
is memory-mapped and demand-paged; measured peak RSS 728\,MB across all thread
...
the standard LFM2-1.2B Q4\_K\_M distribution (8 of 16 down-projection layers at Q6\_K, the remainder at Q4\_K) was benchmarked on the STMicroelectronics
STM32MP257F-DK, which integrates two Cortex-A35 cores running at up to 1.5\,GHz.
...
(i)~SoC rail power: datasheet characterization currents~\cite{ds14284}
(Tables~22--26, $T_j = 25\,^\circ$C) multiplied by the PMIC output voltages
programmed in the board user manual~\cite{um3385} (\S7.5.3, VDDCPU~$=$~0.91\,V, VDDCORE~$=$~0.82\,V);
this yields 174\,mW at T1 and 320\,mW at T2.
(ii)~LPDDR4 power: background-dominated at fewer than 5\% utilization
($\approx$150\,mW independent of thread count).
(iii)~Board peripheral idle power (GigE PHY, STLINK, Wi-Fi module,
eMMC): $\approx$205\,mW, constant across T1 and T2.
(iv)~PMIC buck conversion losses at the operating load, assuming
$\eta = 0.86$ for SoC/DRAM rails and $\eta = 0.88$ for the 3.3\,V
peripheral rail.
Summing all terms gives total board input power of 0.62\,W (T1) and 0.80\,W (T2).
...
The A35 achieves \textbf{1.28}\,t/s decode at T2, versus \textbf{1.66}\,t/s for
...
configurations that enter the acceptable band, and approaching the 1\,s threshold
...
Of the eight KV260 configurations, four enter this band: T3 and T4 CPU Only
...
whereas CPU Only requires three or four to enter the same latency band.
...
require either a faster platform or accepting the ``immediate'' perceptual band
...
Compute (GOPs/s)      & --- & 3.12 & 22.83 & 2.58--2.76\textsuperscript{a} \\
\hline
LUT (\%)          & 30 & 60 & 84 & 87 \\
DSP (\%)          & 27 & 83 & 83 & 20 \\
BRAM (\%)         & 37 & 88 & 71 & 32 \\
...
matrix multiply in isolation. Stage-level peak: 8.00\,GOPs/s (stages~2a/2b, 32 parallel chains); 2.00\,GOPs/s (stage~4, 8 chains per group).}
...
Eleven distinct architectural variants were synthesised and evaluated during
development, systematically varying nibble storage width, BRAM banking strategy,
