...
categorised as immediate and remain acceptable for sustained dialogue,
...
scales and minima (sc6, mn6) that Q4\_K\_M (Q4\_K\_M hereafter) normally stores interleaved at bit-level
...
A structurally simpler quantisation format such as Q4\_0 (one fp16 block scale per
32 elements, no sub-block scales, no minimum subtraction) would achieve the same
effect without CPU-side pre-processing, mapping directly to a single DSP48E2 per
MAC, but would require requantising the model and a separate perplexity validation
step.
The default GGUF distribution of LFM2-1.2B applies mixed quantization to the
down-projection weights, using Q6\_K for eight layers and Q4\_K\_M for the remaining
eight.
...
parallelism---exceeding the device's remaining routing capacity at 88\% LUT
occupancy.
The model is therefore re-quantized to uniform Q4\_K\_M across all sixteen layers.
This choice is consistent with the hardware-in-the-loop co-design process of LFM2,
whose architecture search evaluated all candidates under Q4.0
quantization~\cite{amini2025lfm2technicalreport}---a scheme coarser than Q4\_K\_M in
both bit-width and block-scale granularity.
...
relative to the CPU's FP32 reference is dominated by the Q4\_K\_M weight quantisation
...
The 2048-element INT8 input vector is burst-loaded from DDR in 128~128-bit AXI
...
The stage is pure data movement with no arithmetic; it saturates the 128-bit AXI
bus at \SI{4.00}{GB/s} and completes in 202 cycles (\SI{0.81}{\micro\second}).
...
element-major nibble words; the FPGA never handles the interleaved Q4\_K\_M header
format.
With $K=4$, four rows are processed per iteration (four sequential 80-word AXI
bursts of 16 header and 64 nibble words each).
Nibble extraction uses compile-time \texttt{.range()} indexing, eliminating
the runtime multiplexer trees that dominated LUT consumption in the earlier
K=2 design.
The 32 parallel INT32 MAC chains (8 blocks $\times$ 4 rows) deliver
\SI{1.62}{Gops/s} (20\% of \SI{8.00}{Gops/s} peak), limited by sequential
per-row loads that consume 75\% of the 10.40\,ms iteration (948 of 1,270 cycles).
DDR bandwidth reaches \SI{962}{MB/s} (24\% of the \SI{4.00}{GB/s} peak) across
...
At 16,553 cycles (\SI{66}{\micro\second}) and approximately 65K FP operations
...
organised into 4 sequential groups of 8; all four rows are UNROLL'd per group
to share 32 INT32 MAC chains across 1,024 MAC cycles.
DDR bandwidth reaches \SI{1.60}{GB/s} (40\% of \SI{4.00}{GB/s} peak) for the
\SI{10.0}{MB} transfer (320 words per row $\times$ 2,048 rows).
The stage is compute-bound (MAC 1,024 cycles vs.\ load 476 cycles per row set),
with the 32-way LUTRAM mux in the header decode and 32-block MAC routing
consuming approximately 44,000 LUTs.
Total latency is 1,567,818 cycles (\SI{6.27}{ms}).
...
% Stage 2
\node[stageWorst, below=7mm of x1buf] (x1) {\textbf{Stage 2a}\\$X_1=xW_{gate}$\\2.60 Million cycles / 10.40\,ms};
\node[stageWorst, below=7mm of x2buf] (x2) {\textbf{Stage 2b}\\$X_2=xW_{up}$\\2.60 Million cycles / 10.40\,ms};
...
\node[stageStd, below=7mm of x1cache] (silu) {\textbf{Stage 3}\\SiLU($X_1$)\\16.6 Thousand cycles / 66\,\textmu s};

% Stage 4 (CENTERED BETWEEN BRANCHES)
\node[stageLow, below=10mm of $(silu)!0.5!(x2cache)$] (gate)
{\textbf{Stage 4}\\gate=SiLU($X_1$)$\cdot X_2$\\16.6 Thousand cycles / 66\,\textmu s};

% Stage 5
\node[stageStd, below=of gate] (down)
{\textbf{Stage 5}\\$gate \cdot W_{down}^{T}$\\1.57 Million cycles / 6.27\,ms};
...
\textbf{Software Stack.} The inference engine is based on a modified version of \texttt{llama.cpp}, extended to support hardware offloading of the SwiGLU block. Models are executed using Q4\_K\_M quantization format. The FPGA accelerator was synthesized using Vitis high-level synthesis (HLS) toolchain.
...
Peak resident memory is approximately 660\,MB for CPU execution (the GGUF model
is memory-mapped and demand-paged) and 1.3\,GB for the FPGA+CPU path (adding
the 640\,MB udmabuf DMA buffer).
Per-token decode latency follows from the reciprocal of the throughput values:
from 1.03\,s at T1 CPU-only to 398\,ms at T4 CPU-only, and from 654\,ms at T1
FPGA+CPU to 493\,ms at T2 FPGA+CPU.

\subsection{Comparison with STM32MP257F-DK}
To contextualise the KV260 results against a different ARM-class edge platform,
the same model (Q4\_K\_M) was benchmarked on the STMicroelectronics
STM32MP257F-DK, which integrates two Cortex-A35 cores running at up to 1.5\,GHz.
The A35 is an in-order ARMv8-A microarchitecture optimised for area and power
efficiency, with a 128-bit NEON unit and a 512\,KB shared L2 cache across both cores.
Table~\ref{tab:stm32_results} reports prefill and decode throughput at T1 and T2.

\begin{table}[htbp]
\centering
\scriptsize
\caption{STM32MP257F-DK (2$\times$ Cortex-A35) throughput, Q4\_K\_M model.}
\label{tab:stm32_results}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{c c c}
\hline
Threads & Prefill pp12 (t/s) & Decode tg32 (t/s) \\
\hline
T1 & 0.83 & 0.67 \\
T2 & 1.61 & 1.28 \\
\hline
\end{tabular}
\end{table}
Power was not logged in software; it is estimated from SoC datasheet
characterization tables (DS14284 Rev~5, Table~22--26) at 1.5\,GHz, board PMIC
voltage rails from UM3385~\S7.5.3, a background LPDDR4 model, and the
measured benchmark throughput for DRAM activity (T1: 0.62\,W; T2: 0.80\,W).
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.65\linewidth]{images/utiliz.png}
...
out-of-order Cortex-A53 microarchitecture and the FPGA fabric from the simpler
in-order A35 baseline.
The A35 achieves \textbf{1.28}\,t/s decode at T2, versus \textbf{1.07}\,t/s for
the KV260 A53 at T2 (CPU Only) and \textbf{2.03}\,t/s for the KV260 FPGA+CPU path.
This places the KV260 FPGA+CPU configuration at a \textbf{1.6$\times$} decode
speedup over the A35 at matched thread count, at the cost of a more complex
integration and approximately \textbf{4.0}\,W of additional board power
(4.83\,W FPGA+CPU versus 0.80\,W for the STM32 at T2).
