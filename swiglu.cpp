...
// ============================================================================
// Q4_0 WV Merged Load — K_WV=16 rows, one AXI burst, sub-group-major DDR
// ============================================================================

static void load_16_rows_wv_q40(
    ...
    ap_uint<128> nib_r14[64], ap_uint<128> nib_r15[64],
...
// ============================================================================
// Q4_0 WV MAC — K_WV=16, 1-block sub-group, DSP48E2, II=1 target
// ============================================================================
...
static void mac_blocks_wv_k16_q40(
    ...
    const ap_uint<128> nib_r14[64], const ap_uint<128> nib_r15[64],
...
#pragma HLS BIND_OP op=mul impl=dsp
    ...
    #pragma HLS ARRAY_PARTITION variable=dsp14 complete
    ...
    MAC_GROUPS: for (int g = 0; g < Q40_WV_GROUPS; g++) {
        ...
        MAC_ALL: for (int n = 0; n < 256; n++) {
            ...
            ap_uint<4> nb14 = w14.range(e_lo*8 + bo + 3, e_lo*8 + bo);
            ap_uint<4> nb15 = w15.range(e_lo*8 + bo + 3, e_lo*8 + bo);
            ...
            dsp14[b] += xi8 * (ap_int<18>)((ap_int<5>)nb14 - 8);
        ...
        REDUCE_GRP: for (int b = 0; b < 8; b++) {
            ...
            total14 += (dsp_acc_t)((ap_int<27>)dsp14[b] * d[14][abs_b]);
            total15 += (dsp_acc_t)((ap_int<27>)dsp15[b] * d[15][abs_b]);
        }
    }
